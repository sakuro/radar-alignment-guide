...
describe("Grid.visible_chunk_range", function()
  ...
  it("combines a non-origin position with a non-1 zoom", function()
    local range = Grid.visible_chunk_range({x = 100, y = -50}, {width = 1920, height = 1080}, 0.5)
    assert.are.same({left = 1, right = 5, top = -3, bottom = -1}, range)
  end)

  it("narrows the range at a higher zoom", function()
    local zoomed_out = Grid.visible_chunk_range({x = 15, y = 15}, {width = 1920, height = 1080}, 1)
    local zoomed_in = Grid.visible_chunk_range({x = 15, y = 15}, {width = 1920, height = 1080}, 2)
    assert.are.same({left = -1, right = 1, top = -1, bottom = 0}, zoomed_out)
    assert.are.same({left = 0, right = 0, top = 0, bottom = 0}, zoomed_in)
  end)

  it("clamps the range at an extreme zoom-out", function()
    local range = Grid.visible_chunk_range({ x = 0, y = 0 }, { width = 1e6, height = 1e6 }, 1e-4)
    assert.is_true(range.right - range.left <= 128)
    assert.is_true(range.bottom - range.top <= 128)
  end)
end)
